...
8. [Phase 2: コスト見積もり](#8-phase-2-コスト見積もり)
9. [Phase 3: 申請書作成](#9-phase-3-申請書作成)
10. [Phase 3b: 申請書レビュー](#10-phase-3b-申請書レビュー)
11. [Phase 4: 応募手続き](#11-phase-4-応募手続き)
12. [Phase 5: システム構成図生成（draw.io MCP）](#12-phase-5-システム構成図生成drawio-mcp)
13. [Phase 6: Bicep / CI/CD 生成](#13-phase-6-bicep--cicd-生成)
14. [Phase 7: 採択後管理](#14-phase-7-採択後管理)
15. [プロンプト設計パターン集](#15-プロンプト設計パターン集)
...
9つのサブスキルと2つのカスタムエージェントで構成される GitHub Copilot Agent Skills スイートです。研究者の漠然とした構想から、具体的な申請書・インフラコードまでを自動生成します。
...
flowchart TD
    ...
    Approve3 -->|承認| Phase2[Phase 2: cost-estimator<br/>Azure Retail Prices API]
    ...
    Checklist --> Phase5[Phase 5: diagram-generator<br/>draw.io 構成図]
    Phase5 --> Diagrams[output/diagrams/]
    
    Diagrams --> Phase6[Phase 6: iac-deployer<br/>Bicep / CI/CD]
    Phase6 --> IaC[output/phase4-iac/]
    
    IaC --> Phase7[Phase 7: post-award<br/>採択後管理]
    Phase7 --> Roadmap[output/post-award-roadmap.md]
...
```
Pre  ⏸️→ Phase 0 ⏸️→ Phase 1 ⏸️→ Phase 2 → Phase 3 ⏸️→ Phase 3b ⏸️→ Phase 4 → Phase 5 → Phase 6 → Phase 7
```
...
| 5 | `diagram-generator` | Azure構成設計書 | `output/diagrams/*.drawio` |
| 6 | `iac-deployer` | Azure構成設計書 | `phase4-iac/` |
| 7 | `post-award` | 採択課題情報 | `post-award-roadmap.md` |

### 4.2 緊急度別ワークフロー

| 緊急度 | キーワード | 実行フェーズ |
|--------|----------|------------|
| Normal | （デフォルト） | Pre → Phase 0〜7 |
...
## 12. Phase 5: システム構成図生成（draw.io MCP）

### 12.1 このフェーズの目的

draw.io MCP Server を使用して、Azure 研究基盤のプロフェッショナルなシステム構成図・データフロー図・ネットワーク図を `.drawio` 形式で生成します。Phase 1 の Mermaid 図を補完し、Azure 公式アイコンを使った詳細な図面を作成します。

### 12.2 起動スキル

`spread1000-diagram-generator`

### 12.3 前提条件: draw.io MCP Server のセットアップ
...
| CI/CD 図 | `output/diagrams/cicd-pipeline.drawio` | Phase 6 の Bicep デプロイフロー |
...
├── diagrams/                      # [Ph.5]  draw.io 図面
│   ├── system-architecture.drawio # [Ph.5]  システム構成図
│   ├── data-flow.drawio           # [Ph.5]  データフロー図
│   ├── network-security.drawio    # [Ph.5]  ネットワーク図
│   └── cicd-pipeline.drawio       # [Ph.5]  CI/CD 図
├── phase4-iac/                    # [Ph.6]  IaC 成果物
│   ├── main.bicep
│   ├── modules/
│   │   ├── network.bicep
│   │   ├── storage.bicep
...
│       ├── deploy.yml
│       └── validate.yml
├── post-award-roadmap.md          # [Ph.7]  採択後ロードマップ
├── grant-application-checklist.md # [Ph.7]  交付申請チェックリスト
├── progress-report-interim.md     # [Ph.7]  中間進捗メモ
├── research-outcome-report.md     # [Ph.7]  研究成果報告書
├── budget-change-guide.md         # [Ph.7]  予算変更ガイド
└── accounting-report.md           # [Ph.7]  会計実績報告書
```
